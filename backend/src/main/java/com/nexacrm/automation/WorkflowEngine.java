...
public class WorkflowEngine {
    ...
    /**
     * Process a workflow trigger event for the current tenant.
     * The tenantId is captured here (on the calling thread) and passed explicitly
     * into the async execution so the ThreadLocal TenantContext is available
     * on the async thread, which does not inherit the caller's ThreadLocal state.
     */
    @Async
    public void processEvent(String trigger, Map<String, Object> context) {
        // Capture tenant from calling thread before the async hand-off.
        Long callerTenantId = TenantContext.currentTenantIdOrNull();
        processEventForTenant(trigger, context, callerTenantId);
    }

    private void processEventForTenant(String trigger, Map<String, Object> context, Long tenantId) {
        if (tenantId == null) {
            log.warn("processEvent called with no tenant context for trigger: {}", trigger);
            return;
        }
        // Restore tenant context on the async thread
        TenantContext.setCurrentTenantId(tenantId);
        try {
            processEventInternal(trigger, context);
        } finally {
            TenantContext.clear();
        }
    }

    private void processEventInternal(String trigger, Map<String, Object> context) {
    ...
    /** Maximum allowed WAIT duration to prevent thread-pool exhaustion (30 seconds). */
    ...
    private String waitStep(String waitText) {
        ...
        if (waitMillis > MAX_WAIT_MILLIS) {
            log.warn("WAIT duration {} ms exceeds maximum {}ms cap — capped to prevent thread-pool exhaustion",
